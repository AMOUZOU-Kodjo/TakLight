...
function DashboardStats({ stats }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <h2 className="text-lg font-semibold text-gray-900">Statistiques médias</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {mediaCards.map((card) => (
          <div key={card.label} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="divide-y divide-gray-200">
        {conversations.map((conv) => (
          <div key={conv.id} className="p-4 flex items-center justify-between hover:bg-gray-50">
            <div className="flex items-center gap-3">
              <MessageSquare className="w-5 h-5 text-gray-400" />
              <div>
                <p className="font-medium">{conv.user1.username} ↔ {conv.user2.username}</p>
                <p className="text-sm text-gray-500 truncate max-w-md">{conv.lastMessage || 'Aucun message'}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <span className="text-sm text-gray-500">{conv.messageCount} messages</span>
              <button onClick={() => onView(conv)} className="p-1.5 hover:bg-gray-100 rounded">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className="flex items-start justify-between group">
            <div className={`max-w-xs px-4 py-2 rounded-lg ${
              msg.sender.id === conv.user1.id ? 'bg-gray-100' : 'bg-primary-100'
            }`}>
          ...
          {activity.recentUsers.map((u) => (
            <div key={u.id} className="p-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500">{new Date(u.createdAt).toLocaleString('fr-FR')}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="divide-y divide-gray-200">
          {activity.recentConversations.map((c) => (
            <div key={c.id} className="p-3">
              ...
              <p className="text-xs text-gray-500">{new Date(c.createdAt).toLocaleString('fr-FR')}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="divide-y divide-gray-200">
          {activity.recentMessages.map((m) => (
            <div key={m.id} className="p-3">
              ...
              <p className="text-xs text-gray-500">{new Date(m.sentAt).toLocaleString('fr-FR')}</p>
            </div>
          ))}
        </div>
      </div>
