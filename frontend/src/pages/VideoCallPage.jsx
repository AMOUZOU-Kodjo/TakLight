    ...
    <div className="h-screen bg-gray-900 flex flex-col">
      <div className="flex-1 relative">
        {callStatus === 'connected' || callStatus === 'reconnecting' ? (
          <>
            ...
            <video ref={localVideoRef} autoPlay playsInline muted className="absolute bottom-4 right-4 w-48 h-36 object-cover rounded-lg shadow-lg border-2 border-white/20 dark:border-gray-700" />
          </>
        ) : (
          <div className="w-full h-full flex flex-col items-center justify-center text-white">
            ...
            <p className="text-2xl font-medium">{otherUserName || 'Appel en cours...'}</p>
            <p className="text-gray-400 mt-2">{callStatus === 'ringing' ? 'Sonnerie...' : 'Connexion...'}</p>
          </div>
        )}
      </div>
      ...
      <div className="absolute top-4 left-4 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 flex items-center gap-3">
        ...
        {callStatus === 'connected' && (
          <span className="text-xs bg-white/20 text-white px-2 py-0.5 rounded-full">{qualityLabel[qualityTier] || 'SD'}</span>
        )}
      </div>

      <div className="bg-gray-800 dark:bg-gray-900 px-6 py-4 flex items-center justify-center gap-4">
        <button
          onClick={toggleMute}
          className={`p-4 rounded-full transition-colors ${isMuted ? 'bg-red-500 hover:bg-red-600' : 'bg-gray-700 dark:bg-gray-600 hover:bg-gray-600 dark:hover:bg-gray-500'}`}
        >
        ...
        <button
          onClick={toggleVideo}
          className={`p-4 rounded-full transition-colors ${isVideoOff ? 'bg-red-500 hover:bg-red-600' : 'bg-gray-700 dark:bg-gray-600 hover:bg-gray-600 dark:hover:bg-gray-500'}`}
        >
